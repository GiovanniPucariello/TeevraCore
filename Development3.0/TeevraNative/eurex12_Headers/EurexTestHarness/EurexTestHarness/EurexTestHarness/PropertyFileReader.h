...
char * propertyFileName = "C:/properties/properties.properties";	

//0 - NOT_INITIALIZED, 1 - INITIALIZED
int initialized = 0;
...
void loadProperties()
{
    ...
    while(fgets(fline, MAXDATASIZE, fp) != NULL)
    {
	   ...
	   if (index<=0) {					//no delimiter, move to next line
		 ...
		 // assuming that the property is entered properly and 
		 // also in some cases spaces would be required as property.
...
/*
* 
* The property is read and converted into an int value.
*/

void getIntProperty(char* property, int* value) {
	char strValue[MAXDATASIZE] = {};
	getProperty(property,strValue);
	printf("\n\nProperty Value        : %s\n", strValue);
	*value = atoi(strValue);
}
