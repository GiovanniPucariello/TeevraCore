...
/**
 * This class does alle the work for the process panel. <p/> It responsible for
 * <ul>
...
public class ProcessPanelWorker implements Runnable {
	...
	public ProcessPanelWorker(AutomatedInstallData idata,
			AbstractUIProcessHandler handler) throws IOException {
	...
	private boolean readSpec() throws IOException {
		...
		try {
			...
			System.err
					.println("Error parsing XML specification for processing.");
		...
		for (IXMLElement job_el : spec.getChildrenNamed("job")) {
			// normally use condition attribute, but also read conditionid to
			// not break older versions.
			String conditionid = job_el.hasAttribute("condition") ? job_el
					.getAttribute("condition") : job_el
					.hasAttribute("conditionid") ? job_el
					.getAttribute("conditionid") : null;
			if ((conditionid != null) && (conditionid.length() > 0)) {
				...
				if ((cond != null) && !cond.isTrue()) {
					...
					// skip, if there is a condition and this condition isn't
					// true
					continue;
				}
			}
			...
			Vector<IXMLElement> forPacks = job_el
					.getChildrenNamed("executeForPack");
			...
			if (OsConstraint.oneMatchesCurrentSystem(constraints)) {
				...
				for (IXMLElement ef : job_el.getChildrenNamed("executefile")) {
					...
					if ((ef_name == null) || (ef_name.length() == 0)) {
						System.err
								.println("missing \"name\" attribute for <executefile>");
						return false;
					}
				...
				for (IXMLElement ef : job_el.getChildrenNamed("executeclass")) {
					...
					if ((ef_name == null) || (ef_name.length() == 0)) {
						System.err
								.println("missing \"name\" attribute for <executeclass>");
						return false;
					}
		...
		for (IXMLElement ef : spec.getChildrenNamed("onFail")) {
			String conditionid = ef.hasAttribute("condition") ? ef
					.getAttribute("condition")
					: ef.hasAttribute("conditionid") ? ef
							.getAttribute("conditionid") : null;
			boolean unlockPrev = ef.hasAttribute("previous") ? Boolean
					.parseBoolean(ef.getAttribute("previous")) : false;
			boolean unlockNext = ef.hasAttribute("next") ? Boolean
					.parseBoolean(ef.getAttribute("next")) : false;
			buttonConfigs.get(Boolean.FALSE).add(
					new ButtonConfig(conditionid, unlockPrev, unlockNext));
		}
		for (IXMLElement ef : spec.getChildrenNamed("onSuccess")) {
			...
			boolean unlockPrev = ef.hasAttribute("previous") ? Boolean
					.parseBoolean(ef.getAttribute("previous")) : false;
			buttonConfigs.get(Boolean.TRUE).add(
					new ButtonConfig(conditionid, unlockPrev, true));
		}
	...
	/**
	 * This is called when the processing thread is activated. <p/> Can also be
	 * called directly if asynchronous processing is not desired.
	 */
	public void run() {
		// ExecuteForPack patch
		...
		if (logfiledir != null) {
			logfiledir = IoHelper.translatePath(logfiledir,
					new VariableSubstitutor(idata.getVariables()));
			...
			String identifier = (new SimpleDateFormat("yyyyMMddHHmmss"))
					.format(new Date());
	...
	private static class ExecutableFile implements Processable {
		...
		public boolean run(AbstractUIProcessHandler handler,
			...
			try {
				...
				OutputMonitor stdoutMon = new OutputMonitor(this.handler, p
						.getInputStream(), false);
				OutputMonitor stderrMon = new OutputMonitor(this.handler, p
						.getErrorStream(), true);
				...
				try {
					...
					if (exitStatus != 0) {
						if (this.handler.askQuestion(
								"Process execution failed", "Continue anyway?",
					...
					p.destroy();
					this.handler
							.emitError("process interrupted", ie.toString());
					return false;
				}
		...
		static public class OutputMonitor implements Runnable {
			...
			public OutputMonitor(AbstractUIProcessHandler handler,
					InputStream is, boolean stderr) {
				this.stderr = stderr;
	...
	private static class ExecutableClass implements Processable {
		...
		public boolean run(AbstractUIProcessHandler aHandler,
			...
			try {
				...
				Method m = procClass.getMethod("run", new Class[] {
						AbstractUIProcessHandler.class, String[].class });

				if (m.getReturnType().getName().equals("boolean")) {
					result = ((Boolean) m.invoke(o, new Object[] { myHandler,
							params })).booleanValue();
				} else {
				...
				myHandler.emitError("Post Processing Error",
						"Security exception thrown when processing class: "
								+ myClassName);
			} catch (ClassNotFoundException e) {
				myHandler.emitError("Post Processing Error",
						"Cannot find processing class: " + myClassName);
			} catch (NoSuchMethodException e) {
				myHandler.emitError("Post Processing Error",
						"Processing class does not have 'run' method: "
								+ myClassName);
			} catch (IllegalAccessException e) {
				myHandler.emitError("Post Processing Error",
						"Error accessing processing class: " + myClassName);
			} catch (InvocationTargetException e) {
				myHandler.emitError("Post Processing Error",
						"Invocation Problem calling : " + myClassName + ", "
								+ e.getCause().getMessage());
			} catch (Exception e) {
				myHandler.emitError("Post Processing Error",
						"Exception when running processing class: "
								+ myClassName + ", " + e.getMessage());
			} catch (Error e) {
				myHandler.emitError("Post Processing Error",
...
class ButtonConfig {
	 ...
	 * @param unlockNext
	 */
	public ButtonConfig(String conditionid, boolean unlockPrev,
			boolean unlockNext) {
