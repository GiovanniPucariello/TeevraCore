	...
	public void parseFiles() throws Exception {
		...
		while (iter.hasNext()) {
			...
			try {
				...
				File parsedFile = new File(java.io.File.createTempFile("izpp",
						null, null));
				...
				outFile = new FileOutputStream(parsedFile);
				 ...
				 * FIX FOR ICEFISH
				 * 
				 * copy parsed file to original file.
				 * and delete the parsed file... doing this as 
				 * rename to isn't working on solaris 10.
				 */
				
				file = new File(pfile.path);
				file.copyFrom(parsedFile);
				parsedFile.delete();
/*				if (!parsedFile.renameTo(file)) {
					throw new IOException("Could not rename file " + parsedFile
							+ " to " + file);
				}
