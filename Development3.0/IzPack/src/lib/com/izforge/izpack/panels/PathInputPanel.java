...
public class PathInputPanel extends IzPanel implements ActionListener {
	...
	private String panelName = "TargetPanel";
	...
	public PathInputPanel(InstallerFrame parent, InstallData idata) {
		...
		// Label for input
		// row 1 column 0.
		add(createLabel("info", panelName, "open", LEFT, true), NEXT_LINE);
	...
	public boolean isValidated() {
		...
		boolean modifyinstallation = Boolean.valueOf(idata
				.getVariable(InstallData.MODIFY_INSTALLATION));
		if (modifyinstallation) {
			// installation directory has to exist in a modification installation
			mustExist = true;

			File installationinformation = new File(pathSelectionPanel
					.getPath()
					+ File.separator
					+ AutomatedInstallData.INSTALLATION_INFORMATION);
		...
		if (chosenPath.length() == 0) {
			...
			ok = emitWarning(parent.langpack.getString("installer.warning"),
					emptyTargetMsg);
		}
		if (!ok) {
			return ok;
		...
		if (isMustExist()) {
			if (!path.exists()) {
				emitError(parent.langpack.getString("installer.error"),
						parent.langpack.getString(getI18nStringForClass(
								"required", "PathInputPanel")));
				return false;
			}
			if (!pathIsValid()) {
				emitError(parent.langpack.getString("installer.error"),
						parent.langpack.getString(getI18nStringForClass(
								"notValid", "PathInputPanel")));
				return false;
			}
		} else {
			...
			if (path.exists()) {
				int res = askQuestion(parent.langpack
						.getString("installer.warning"), warnMsg,
				...
				final String vStr = idata
						.getVariable("ShowCreateDirectoryMessage");
	...
	protected boolean pathIsValid() {
		...
		for (String existFile : existFiles) {
			File path = new File(pathSelectionPanel.getPath(), existFile)
					.getAbsoluteFile();
			if (!path.exists()) {
				return false;
			}
