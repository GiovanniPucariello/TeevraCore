package  com.headstrong.fusion.bo.java.generated.fixml_5_0;

import javax.annotation.Generated;

import com.headstrong.fusion.bo.java.BeanAccessor;
import org.fixprotocol.fixml_5_0.RootSubPartiesBlockT;

import com.headstrong.fusion.bo.impl.exception.AttributeNotFoundException;
import com.headstrong.fusion.bo.java.JavaBusinessObject;

//This file was generated on Wed Jan 20 15:18:35 IST 2010
@Generated(value = { "bo-accessor-generator" }, date="Wed Jan 20 15:18:35 IST 2010")
public class RootSubPartiesBlockTBeanAccessor implements BeanAccessor<RootSubPartiesBlockT> {
	
	private RootSubPartiesBlockT bean;
	
	@Override
	public Object get(String accessPath) throws AttributeNotFoundException {
		 if("rtPtySubID".equalsIgnoreCase(accessPath)){
			return this.bean.getRtPtySubID();
		}
		else if("rtPtySubIDTyp".equalsIgnoreCase(accessPath)){
			return this.bean.getRtPtySubIDTyp();
		}
		throw new AttributeNotFoundException(accessPath, new JavaBusinessObject(bean));
	}

	@Override
	public void set(String accessPath, Object obj)throws AttributeNotFoundException {
		 if("rtPtySubID".equalsIgnoreCase(accessPath)){
			this.bean.setRtPtySubID((String) obj);
			return;
		}
		else if("rtPtySubIDTyp".equalsIgnoreCase(accessPath)){
			this.bean.setRtPtySubIDTyp((Integer) obj);
			return;
		}
		throw new AttributeNotFoundException(accessPath, new JavaBusinessObject(bean));
	}

	@Override
	public void setTarget(RootSubPartiesBlockT obj) {
		this.bean = obj; 
	}

}
