 ...
 * 
 */
public class DbErrorFormatterTest extends TestCase {
	...
	public void testFormatSimpleResource() throws Exception {
		...
		List<String> errorRecord = (List<String>) errorFormatter.format(errCtx);

		assertEquals("System", errorRecord.get(0));
		assertEquals("validator_1", errorRecord.get(1));
		assertEquals("validator", errorRecord.get(2));
		assertTrue(errorRecord.get(4).contains("FusionException"));
		assertEquals("Resource", errorRecord.get(5));
		assertEquals("Additional Information", errorRecord.get(6));
	}
	...
	public void testFormatResourceAsBusinessObject() throws Exception {
		...
		List<Object> errorRecord = (List<Object>) errorFormatter.format(errCtx);

		assertEquals("System", errorRecord.get(0));
		assertEquals("validator_1", errorRecord.get(1));
		assertEquals("validator", errorRecord.get(2));
		assertTrue(errorRecord.get(4).toString().contains("FusionException"));
		assertEquals(
