 ...
 * All rights reserved.
 * 
 * $Id: DivisionRule.java
 * $Revision: 
 * $Author: Ssoni
 * $DateTime: Feb 15, 2010 
 */
...
public class DivisionRule implements Rule {
	...
	public static final RuleVO getRuleVO() {
		...
		ruleVO.setAliasName("division");
		ruleVO.setClassName(DivisionRule.class.getName());
	...
	public Object operate(Map<String, Object> data)
		...
		try {
			if (logger.isDebugEnabled()) {
				logger.debug("DivisionRule operating on :" + data);
			}
			...
			logger.debug("In divison in DivisionRule--- OPERAND1-->"+operand1+"<--OPERAND2--->"+operand2+"--->PRODUCT is"+product);
			Map<String, Object> resultMap = new HashMap<String, Object>();
		//	resultMap.put(RESULT, nf.format(product));
			resultMap.put(RESULT, product.toString());
			return resultMap;
		} catch (Exception e) {
			logger.error("Error while operating rule division on :" + data);
			throw new RuleOperationFailedException(
					"Error while operating rule division on :", e);
		}
