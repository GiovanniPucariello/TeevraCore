...
/**
 * Rule for Modulus operation
 */
public class ModulusRule implements Rule {

	private static final Logger logger=LoggerFactory.getLogger(ModulusRule.class);
	...
	public static final String OPERAND2=OPERAND+"2";
	  ...
	  public static final RuleVO getRuleVO() {
			...
			ruleVO.setAliasName("modulus");
			ruleVO.setClassName(ModulusRule.class.getName());
	...
	public Object operate(Map<String, Object> data)
		...
		try {
			if (logger.isDebugEnabled()) {
				logger.debug("ModulusRule operating on :"+ data);
			}
		...
		resultMap.put(RESULT,nf.format(modulus));
		return resultMap;
		}
		catch (Exception e) {
			logger.error("Error while operating rule modulus on :"+ data);
			throw new RuleOperationFailedException("Error while operating rule modulus.",e);
		}
