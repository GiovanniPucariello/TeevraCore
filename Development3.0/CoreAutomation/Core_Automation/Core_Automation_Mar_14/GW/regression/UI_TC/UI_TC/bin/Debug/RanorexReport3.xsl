	...
	<tr class="{@level}" style="{@style}">
      ...
      <xsl:if test='./metainfo/@stacktrace'>
        <div class="stacktrace" >
          <span onclick="$(this).next().toggle();">Show Stacktrace</span>
          <p>
...
<xsl:template name="levelFilterSelector">
	<div class="filter">
		<span>Filter: </span>
		<xsl:for-each select=".//item[not(@level=preceding-sibling::item/@level)]">
			<xsl:variable name="currentId"><xsl:value-of select="generate-id(.)"/></xsl:variable>
			<xsl:choose>
				<xsl:when test="../../activity[@type='test module']">
					<input type="checkbox" id="checkBox{$currentId}" name="checkBox{$currentId}" onClick="$(this).parent().parent().next().find('tr.{@level}').toggle();" checked="1" />
				</xsl:when>
				<xsl:otherwise>
					<input type="checkbox" id="checkBox{$currentId}" name="checkBox{$currentId}" onClick="$('tr.{@level}').toggle();" checked="1" />
				</xsl:otherwise>
			</xsl:choose>
			<label for="checkBox{$currentId}"><xsl:value-of select="@level"/></label>
		</xsl:for-each> 
	</div>
</xsl:template> 
